...
async function readPayload(req: NextRequest): Promise<CareersPayload> {
  const contentType = req.headers.get("content-type") || "";
  if (contentType.includes("application/json")) {
    return (await req.json()) as CareersPayload;
  }
  ...
  if (webhook) {
    try {
      await fetch(webhook, {
        ...
        body: JSON.stringify({ type: "careers", ...data, at: new Date().toISOString() }),
      });
    } catch (err) {
      console.error("careers webhook failed", err);
    }
  } else {
    console.log("[careers form]", data);
  }
